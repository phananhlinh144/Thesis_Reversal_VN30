...
st.set_page_config(page_title="VN30 AI PRO - VCI Stealth Mode", layout="wide")
...
# --- 3. AUTO-SCAN (Chiến thuật 1.7s per stock) ---
if 'all_results' not in st.session_state:
    with st.status("🚀 Đang khởi tạo dữ liệu VN30 (Chế độ Stealth - 1.7s/mã)...", expanded=True) as status:
        results = []
        for i, sym in enumerate(vn30_symbols):
            st.write(f"⏳ ({i+1}/30) Đang quét: **{sym}**...")
            ...
            # CHÍNH NÓ: Nghỉ 1.7 giây sau mỗi mã để bypass VCI
            time.sleep(1.7)
            
        st.session_state.all_results = pd.DataFrame(results, columns=["Mã", "Giá HT", "win50", "win10", "ENSEMBLE"])
        status.update(label="✅ Đã quét xong toàn bộ VN30!", state="complete", expanded=False)
