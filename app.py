...
st.set_page_config(page_title="VN30 AI PRO - VCI Optimized", layout="wide")
...
# --- 2. HÀM XỬ LÝ DỮ LIỆU (Sử dụng VCI) ---
@st.cache_data(ttl=3600)
def get_integrated_data(symbol):
    try:
        drive_url = 'https://drive.google.com/uc?id=1xG6J9fBEF_Z4KY3_frUwnhVTSA6HG2r'
        ...
        # Nguồn VCI ổn định hơn cho data realtime
        ...
        try:
            # Dự phòng nếu Drive lỗi, lấy thẳng 300 phiên từ VCI
...
# --- 3. AUTO-SCAN (Có Rate Limiting cho VCI) ---
if 'all_results' not in st.session_state:
    with st.status("🚀 Đang khởi tạo dữ liệu VN30 (VCI Source)...", expanded=True) as status:
        results = []
        for i, sym in enumerate(vn30_symbols):
            # Cứ mỗi 10 mã thì nghỉ 2 giây
            if i > 0 and i % 10 == 0:
                st.write(f"😴 Nghỉ giải lao 2s cho VCI đỡ mệt...")
                time.sleep(2)
            
            st.write(f"⏳ Đang phân tích: {sym} ({i+1}/30)...")
        ...
        status.update(label="✅ Đã quét xong VN30!", state="complete", expanded=False)
